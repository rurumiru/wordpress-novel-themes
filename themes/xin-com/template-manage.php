...
if ( ! xin_can_moderate() ) {
	...
	<div class="xin-wrap">
		<div class="xin-empty" style="padding-block:80px">
			...
			<p><?php esc_html_e( 'Раздел для модераторов и администрации площадки.', 'xin-com' ); ?></p>
			<?php if ( ! is_user_logged_in() ) : ?>
				<div class="xin-mt-2"><a class="btn btn-primary" href="<?php echo esc_url( xin_login_url( get_permalink() ) ); ?>"><?php esc_html_e( 'Войти', 'xin-com' ); ?></a></div>
			<?php endif; ?>
		</div>
	</div>
	<?php
...
$xin_tabs  = array(
	'overview'   => __( 'Обзор', 'xin-com' ),
	'users'      => __( 'Пользователи', 'xin-com' ),
	'moderation' => __( 'Модерация', 'xin-com' ),
	'titles'     => __( 'Тайтлы', 'xin-com' ),
);
if ( xin_can_manage() ) {
	$xin_tabs['settings'] = __( 'Настройки', 'xin-com' );
}
?>
...
<div class="xin-wrap xin-manage">
	...
	<?php if ( 'overview' === $xin_tab ) : ?>
		...
		<div class="xin-manage__cards">
			<div class="xin-manage__card">
				<span><?php esc_html_e( 'Пользователей', 'xin-com' ); ?></span>
			...
			<div class="xin-manage__card">
				<span><?php esc_html_e( 'Глав', 'xin-com' ); ?></span>
			...
			<div class="xin-manage__card<?php echo $xin_stats['pending'] > 0 ? ' is-warn' : ''; ?>">
				<span><?php esc_html_e( 'Ждут проверки', 'xin-com' ); ?></span>
				<b><?php echo esc_html( number_format_i18n( $xin_stats['pending'] ) ); ?></b>
			</div>
		</div>
	...
	<?php elseif ( 'users' === $xin_tab ) : ?>
		...
		<form class="xin-manage__search" method="get" action="<?php echo esc_url( xin_manage_url() ); ?>">
			...
			<input type="search" name="q" value="<?php echo esc_attr( $xin_q ); ?>" placeholder="<?php esc_attr_e( 'Имя, логин или почта', 'xin-com' ); ?>">
			<button class="btn btn-outline btn-sm" type="submit"><?php esc_html_e( 'Найти', 'xin-com' ); ?></button>
			<?php if ( $xin_q ) : ?>
				<a class="btn btn-ghost btn-sm" href="<?php echo esc_url( xin_manage_url( array( 'tab' => 'users' ) ) ); ?>"><?php esc_html_e( 'Сбросить', 'xin-com' ); ?></a>
			<?php endif; ?>
		</form>
		...
		<div class="xin-manage__table">
					...
					<div class="xin-manage__act">
						<?php if ( xin_can_manage() && ! user_can( $xin_user->ID, 'manage_options' ) && get_current_user_id() !== $xin_user->ID ) : ?>
							<form method="post" action="<?php echo esc_url( xin_manage_url() ); ?>">
								...
								<select name="role" aria-label="<?php esc_attr_e( 'Роль', 'xin-com' ); ?>">
								...
								<button class="btn btn-outline btn-sm" type="submit"><?php esc_html_e( 'Сменить', 'xin-com' ); ?></button>
							</form>
						<?php endif; ?>
			...
			<?php if ( ! $xin_users ) : ?>
				<p class="xin-empty-inline"><?php esc_html_e( 'Никого не нашлось.', 'xin-com' ); ?></p>
			<?php endif; ?>
		</div>
		...
		<?php if ( $xin_pages > 1 ) : ?>
			<div class="xin-manage__pager">
				<?php if ( $xin_page > 1 ) : ?>
					<a class="btn btn-outline btn-sm" href="<?php echo esc_url( xin_manage_url( array( 'tab' => 'users', 'q' => $xin_q, 'pg' => $xin_page - 1 ) ) ); ?>"><?php esc_html_e( 'Назад', 'xin-com' ); ?></a>
				<?php endif; ?>
				<span><?php echo esc_html( sprintf( '%d / %d', $xin_page, $xin_pages ) ); ?></span>
				<?php if ( $xin_page < $xin_pages ) : ?>
					<a class="btn btn-outline btn-sm" href="<?php echo esc_url( xin_manage_url( array( 'tab' => 'users', 'q' => $xin_q, 'pg' => $xin_page + 1 ) ) ); ?>"><?php esc_html_e( 'Дальше', 'xin-com' ); ?></a>
				<?php endif; ?>
			</div>
		<?php endif; ?>
	...
	<?php elseif ( 'moderation' === $xin_tab ) : ?>
		...
		<div class="xin-manage__table">
				...
				<div class="xin-manage__row">
						...
						<div>
							...
							<small>
								...
								printf(
									...
									esc_html__( '%1$s · %2$s · %3$s', 'xin-com' ),
									esc_html( 'novel' === $xin_item->post_type ? __( 'Тайтл', 'xin-com' ) : __( 'Глава', 'xin-com' ) ),
					...
					<div class="xin-manage__act">
						<form method="post" action="<?php echo esc_url( xin_manage_url() ); ?>">
							...
							<button class="btn btn-primary btn-sm" type="submit" name="what" value="publish"><?php esc_html_e( 'Опубликовать', 'xin-com' ); ?></button>
							<button class="btn btn-outline btn-sm" type="submit" name="what" value="draft"><?php esc_html_e( 'В черновики', 'xin-com' ); ?></button>
							<button class="btn btn-ghost btn-sm" type="submit" name="what" value="trash"><?php esc_html_e( 'В корзину', 'xin-com' ); ?></button>
						</form>
					</div>
				</div>
			<?php endforeach; ?>
			...
			<?php if ( ! $xin_queue ) : ?>
				<p class="xin-empty-inline"><?php esc_html_e( 'Очередь пуста — проверять нечего.', 'xin-com' ); ?></p>
			<?php endif; ?>
		</div>
	...
	<?php elseif ( 'titles' === $xin_tab ) : ?>
		...
		<div class="xin-manage__table">
			<?php while ( $xin_novels->have_posts() ) : $xin_novels->the_post(); ?>
				<div class="xin-manage__row">
						...
						<div>
							...
							<small>
								...
								printf(
									...
									esc_html( sprintf( _n( '%d глава', '%d глав', xin_chapter_count( get_the_ID() ), 'xin-com' ), xin_chapter_count( get_the_ID() ) ) ),
									esc_html( get_post_status() === 'publish' ? __( 'опубликован', 'xin-com' ) : get_post_status() )
								);
								?>
							</small>
						</div>
					...
					<div class="xin-manage__act">
						<a class="btn btn-outline btn-sm" href="<?php echo esc_url( xin_dashboard_url( array( 'view' => 'chapters', 'project' => get_the_ID() ) ) ); ?>"><?php esc_html_e( 'Главы', 'xin-com' ); ?></a>
						<form method="post" action="<?php echo esc_url( xin_manage_url() ); ?>">
							<?php wp_nonce_field( 'xin_manage_moderate' ); ?>
							<input type="hidden" name="xin_manage" value="moderate">
							...
							<?php if ( 'publish' === get_post_status() ) : ?>
								<button class="btn btn-outline btn-sm" type="submit" name="what" value="draft"><?php esc_html_e( 'Скрыть', 'xin-com' ); ?></button>
							<?php else : ?>
								<button class="btn btn-primary btn-sm" type="submit" name="what" value="publish"><?php esc_html_e( 'Опубликовать', 'xin-com' ); ?></button>
							<?php endif; ?>
						</form>
			...
			<?php if ( ! $xin_novels->found_posts ) : ?>
				<p class="xin-empty-inline"><?php esc_html_e( 'Тайтлов пока нет.', 'xin-com' ); ?></p>
			<?php endif; ?>
		</div>
		...
		<?php if ( $xin_novels->max_num_pages > 1 ) : ?>
			<div class="xin-manage__pager">
				<?php if ( $xin_page > 1 ) : ?>
					<a class="btn btn-outline btn-sm" href="<?php echo esc_url( xin_manage_url( array( 'tab' => 'titles', 'pg' => $xin_page - 1 ) ) ); ?>"><?php esc_html_e( 'Назад', 'xin-com' ); ?></a>
				<?php endif; ?>
				<span><?php echo esc_html( sprintf( '%d / %d', $xin_page, $xin_novels->max_num_pages ) ); ?></span>
				<?php if ( $xin_page < $xin_novels->max_num_pages ) : ?>
					<a class="btn btn-outline btn-sm" href="<?php echo esc_url( xin_manage_url( array( 'tab' => 'titles', 'pg' => $xin_page + 1 ) ) ); ?>"><?php esc_html_e( 'Дальше', 'xin-com' ); ?></a>
				<?php endif; ?>
			</div>
		<?php endif; ?>
	...
	<?php else : ?>
		...
		<form class="xin-manage__settings" method="post" action="<?php echo esc_url( xin_manage_url() ); ?>">
			...
			<label class="xin-manage__toggle">
				...
				<span>
					<b><?php esc_html_e( 'Открытая регистрация', 'xin-com' ); ?></b>
					<small><?php esc_html_e( 'Форма на странице входа принимает новых читателей.', 'xin-com' ); ?></small>
				</span>
			</label>
			...
			<label class="xin-manage__toggle">
				...
				<span>
					<b><?php esc_html_e( 'Обсуждения', 'xin-com' ); ?></b>
					<small><?php esc_html_e( 'Комментарии под главами и тайтлами. По умолчанию выключены.', 'xin-com' ); ?></small>
				</span>
			</label>

			<label class="xin-manage__field">
				<span><?php esc_html_e( 'Кем становится новый пользователь', 'xin-com' ); ?></span>
				<select name="new_user_role">
					<option value="author" <?php selected( xin_new_user_role(), 'author' ); ?>><?php esc_html_e( 'Автор', 'xin-com' ); ?></option>
					<option value="contributor" <?php selected( xin_new_user_role(), 'contributor' ); ?>><?php esc_html_e( 'Участник', 'xin-com' ); ?></option>
					<option value="subscriber" <?php selected( xin_new_user_role(), 'subscriber' ); ?>><?php esc_html_e( 'Читатель', 'xin-com' ); ?></option>
				</select>
			</label>

			<label class="xin-manage__field">
				<span><?php esc_html_e( 'Скачивание книг', 'xin-com' ); ?></span>
			...
			<div class="xin-manage__field">
				<span><?php esc_html_e( 'Роли со скачиванием', 'xin-com' ); ?></span>
				...
				<small><?php esc_html_e( 'Учитываются в двух последних режимах. Достаточно одной отмеченной роли; администратор скачивает всегда.', 'xin-com' ); ?></small>
			</div>

			<label class="xin-manage__field">
				<span><?php esc_html_e( 'Основной язык', 'xin-com' ); ?></span>
			...
			<label class="xin-manage__field">
				<span><?php esc_html_e( 'Схема по умолчанию', 'xin-com' ); ?></span>
				<select name="default_scheme">
					<option value="light" <?php selected( get_theme_mod( 'xin_default_scheme', 'light' ), 'light' ); ?>><?php esc_html_e( 'Светлая', 'xin-com' ); ?></option>
					<option value="dark" <?php selected( get_theme_mod( 'xin_default_scheme', 'light' ), 'dark' ); ?>><?php esc_html_e( 'Тёмная', 'xin-com' ); ?></option>
					<option value="auto" <?php selected( get_theme_mod( 'xin_default_scheme', 'light' ), 'auto' ); ?>><?php esc_html_e( 'Как в системе', 'xin-com' ); ?></option>
				</select>
			</label>

			<button class="btn btn-primary" type="submit"><?php esc_html_e( 'Сохранить', 'xin-com' ); ?></button>
		</form>
