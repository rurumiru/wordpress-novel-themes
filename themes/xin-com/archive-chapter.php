...
<div class="xin-aurora">
	<div class="xin-wrap">
		<header class="xin-pagehead">
			<?php xin_breadcrumbs(); ?>
			<span class="xin-eyebrow"><?php esc_html_e( 'лента', 'xin-com' ); ?></span>
			<h1><?php esc_html_e( 'Обновления', 'xin-com' ); ?></h1>
			<p class="xin-pagehead__sub"><?php esc_html_e( 'Свежие главы со всей площадки — в порядке публикации.', 'xin-com' ); ?></p>
		</header>
	</div>
</div>
...
<div class="xin-wrap xin-mt-3">
			...
			while ( have_posts() ) :
				...
				if ( $xin_day !== $xin_current ) {
					...
					if ( $xin_today === $xin_day ) {
						$xin_title = __( 'Сегодня', 'xin-com' );
					} elseif ( $xin_yesterday === $xin_day ) {
						$xin_title = __( 'Вчера', 'xin-com' );
					} else {
	...
	<?php else : ?>
		<div class="xin-empty">
			...
			<h2><?php esc_html_e( 'Обновлений пока нет', 'xin-com' ); ?></h2>
			<p><?php esc_html_e( 'Как только выйдет первая глава, она появится здесь.', 'xin-com' ); ?></p>
		</div>
	<?php endif; ?>
</div>
