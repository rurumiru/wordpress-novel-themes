...
if ( ! $xin_novel_id || ! xin_owns( $xin_novel_id ) ) {
	echo '<p class="xin-empty-inline">' . esc_html__( 'Сначала выберите проект.', 'xin-com' ) . '</p>';
	return;
}
...
<form class="xin-panel" method="post" action="<?php echo esc_url( xin_dashboard_url() ); ?>">
	...
	<div class="xin-panel__head">
		<h2>
			...
			<?php esc_html_e( 'Словарь проекта', 'xin-com' ); ?>
		</h2>
	...
	<div class="xin-form">
		<p class="xin-field__hint">
			<?php esc_html_e( 'Имена и термины, которые читатель увидит вместо того, что стоит в тексте. Замена идёт в браузере читателя: сам текст глав остаётся как есть, пока вы не нажмёте «Вписать в главы».', 'xin-com' ); ?>
		</p>

		<div class="xin-field">
			<label for="xin-glossary"><?php esc_html_e( 'Правила: одно в строке, «было = стало»', 'xin-com' ); ?></label>
			...
			<p class="xin-field__hint">
				<?php esc_html_e( 'Пустая правая часть убирает термин из текста. Строка, начатая с #, — комментарий.', 'xin-com' ); ?>
			</p>
		</div>
		...
		<div class="xin-field">
			<div class="xin-checks">
				<label class="xin-check">
					...
					<?php esc_html_e( 'Любой регистр', 'xin-com' ); ?>
				</label>
				<label class="xin-check">
					<input type="checkbox" name="whole" value="1" <?php checked( $xin_whole ); ?>>
					<?php esc_html_e( 'Слово целиком', 'xin-com' ); ?>
				</label>
			</div>
			<p class="xin-field__hint">
				<?php esc_html_e( 'Настройки действуют на все правила проекта. «Слово целиком» бережёт склонения: «Чэн» не тронет «Чэня».', 'xin-com' ); ?>
			</p>
		</div>

		<div class="xin-flex xin-flex-wrap">
			<button type="submit" class="btn btn-primary btn-lg">
				<?php xin_the_icon( 'check' ); ?><?php esc_html_e( 'Сохранить словарь', 'xin-com' ); ?>
			</button>
			<span class="xin-w__note">
				<?php
				printf(
					/* translators: 1: rules, 2: chapters */
					esc_html__( 'Правил: %1$s · глав в проекте: %2$s', 'xin-com' ),
...
<?php if ( $xin_rules ) : ?>
	<div class="xin-panel xin-mt-2">
		<div class="xin-panel__head">
			<h2><?php xin_the_icon( 'wand' ); ?><?php esc_html_e( 'Вписать словарь в главы', 'xin-com' ); ?></h2>
		</div>

		<div class="xin-form">
			<p class="xin-field__hint">
				<?php esc_html_e( 'Это меняет сам текст глав в базе — как будто вы прошли по ним поиском и заменой. Сначала посмотрите, сколько совпадений найдётся.', 'xin-com' ); ?>
			</p>
				...
				<p class="xin-notice xin-notice--ok" style="margin:0">
					...
					<span>
						...
						printf(
							...
							esc_html__( 'Совпадений: %1$s в %2$s главах.', 'xin-com' ),
			...
			<div class="xin-flex xin-flex-wrap">
				<form method="post" action="<?php echo esc_url( xin_dashboard_url() ); ?>">
					...
					<button type="submit" class="btn btn-outline">
						<?php xin_the_icon( 'search' ); ?><?php esc_html_e( 'Посчитать совпадения', 'xin-com' ); ?>
					</button>
				</form>
				...
				<form method="post" action="<?php echo esc_url( xin_dashboard_url() ); ?>" class="xin-flex xin-flex-wrap">
					...
					<label class="xin-check">
						...
						<?php esc_html_e( 'Понимаю: текст глав изменится', 'xin-com' ); ?>
					</label>
					<button type="submit" class="btn btn-gold">
						<?php xin_the_icon( 'wand' ); ?><?php esc_html_e( 'Вписать в главы', 'xin-com' ); ?>
					</button>
				</form>
			</div>
		</div>
