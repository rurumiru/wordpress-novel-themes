...
$xin_units = array(
	'rating'   => __( 'оценка', 'xin-com' ),
	'views'    => __( 'просмотров', 'xin-com' ),
	'chapters' => __( 'глав', 'xin-com' ),
);
...
<div class="xin-aurora">
	<div class="xin-wrap">
		<header class="xin-pagehead">
			<?php xin_breadcrumbs(); ?>
			<span class="xin-eyebrow"><?php esc_html_e( 'доска', 'xin-com' ); ?></span>
			<h1><?php the_title(); ?></h1>
			<p class="xin-pagehead__sub">
				<?php esc_html_e( 'Лучшее на площадке — по оценке читателей, по числу просмотров и по объёму. Пересчитывается автоматически.', 'xin-com' ); ?>
			</p>
		</header>
	</div>
</div>
...
<div class="xin-wrap xin-rank">
	...
	<div class="xin-rank__controls">
		<div class="xin-segmented" role="tablist" aria-label="<?php esc_attr_e( 'Показатель', 'xin-com' ); ?>">
		...
		<div class="xin-segmented" role="tablist" aria-label="<?php esc_attr_e( 'Период', 'xin-com' ); ?>">
	...
	<?php if ( ! is_wp_error( $xin_genres ) && $xin_genres ) : ?>
		<div class="xin-genres xin-mt-2">
			<a class="xin-genre-chip <?php echo $xin_state['genre'] ? '' : 'is-active'; ?>" href="<?php echo esc_url( xin_ranking_url( array( 'genre' => '' ) ) ); ?>">
				<?php esc_html_e( 'Все жанры', 'xin-com' ); ?>
			</a>
	...
	<?php if ( ! $xin_board ) : ?>
		...
		<div class="xin-empty" style="padding-block:70px">
			<h2><?php esc_html_e( 'Здесь пока пусто', 'xin-com' ); ?></h2>
			<p><?php esc_html_e( 'Ни один тайтл не набрал показателей для этой доски. Попробуйте другой период или снимите фильтр по жанру.', 'xin-com' ); ?></p>
					...
					<span style="min-width:0">
						...
						<span class="xin-rank__value">
							...
							<?php if ( 'rating' === $xin_metric && $xin_row['votes'] ) : ?>
								<small><?php printf( esc_html__( '· голосов: %s', 'xin-com' ), esc_html( number_format_i18n( $xin_row['votes'] ) ) ); ?></small>
							<?php endif; ?>
						</span>
		...
		<?php if ( 'rating' === $xin_metric ) : ?>
			<p class="xin-rank__note">
				...
				printf(
					...
					esc_html__( 'Оценка взвешенная: пока у тайтла меньше %s голосов, его средняя подтягивается к средней по площадке. Так одна пятёрка не обгоняет сотню честных оценок.', 'xin-com' ),
