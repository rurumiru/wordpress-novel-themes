...
<div class="xin-wrap">
	...
	<?php if ( ! is_user_logged_in() ) : ?>
		...
		<div class="xin-empty" style="padding-block:80px">
			...
			<h1><?php esc_html_e( 'Кабинет автора', 'xin-com' ); ?></h1>
			<p><?php esc_html_e( 'Войдите под своей учётной записью, чтобы вести проекты и публиковать главы.', 'xin-com' ); ?></p>
			<div class="xin-flex xin-flex-wrap xin-mt-2" style="justify-content:center">
				...
				<?php if ( xin_registration_open() ) : ?>
					<a class="btn btn-outline" href="<?php echo esc_url( xin_register_url( get_permalink() ) ); ?>"><?php esc_html_e( 'Создать аккаунт', 'xin-com' ); ?></a>
				<?php endif; ?>
			</div>
		</div>
	...
	<?php elseif ( ! xin_can_author() ) : ?>
		...
		<div class="xin-empty" style="padding-block:80px">
			...
			<h1><?php esc_html_e( 'Кабинет автора', 'xin-com' ); ?></h1>
			<p><?php esc_html_e( 'Учётная запись есть, но прав на публикацию у неё пока нет. Их выдаёт администрация площадки — напишите, и кабинет откроется.', 'xin-com' ); ?></p>
			<div class="xin-flex xin-flex-wrap xin-mt-2" style="justify-content:center">
				<a class="btn btn-primary" href="<?php echo esc_url( xin_page_url( 'become-author' ) ); ?>"><?php esc_html_e( 'Как стать автором', 'xin-com' ); ?></a>
				<a class="btn btn-outline" href="<?php echo esc_url( xin_page_url( 'contacts' ) ); ?>"><?php esc_html_e( 'Связаться', 'xin-com' ); ?></a>
			</div>
		</div>
	...
	<?php else : ?>
		...
		<header class="xin-pagehead">
			<?php xin_breadcrumbs(); ?>
			<h1><?php esc_html_e( 'Кабинет автора', 'xin-com' ); ?></h1>
			<p class="xin-pagehead__sub"><?php esc_html_e( 'Проекты, главы и черновики — всё редактируется прямо здесь.', 'xin-com' ); ?></p>
		</header>
		...
		<div class="xin-dash">
			...
			<nav class="xin-dash__nav">
				<a href="<?php echo esc_url( xin_dashboard_url() ); ?>" class="<?php echo 'novels' === $xin_view ? 'is-active' : ''; ?>">
					<?php xin_the_icon( 'library' ); ?><?php esc_html_e( 'Мои проекты', 'xin-com' ); ?>
				</a>
				...
				<a href="<?php echo esc_url( get_author_posts_url( get_current_user_id() ) ); ?>">
					<?php xin_the_icon( 'user' ); ?><?php esc_html_e( 'Мой профиль', 'xin-com' ); ?>
				</a>
				<a href="<?php echo esc_url( xin_library_url() ); ?>">
					<?php xin_the_icon( 'bookmark' ); ?><?php esc_html_e( 'Моя библиотека', 'xin-com' ); ?>
				</a>
				<a href="<?php echo esc_url( admin_url() ); ?>">
					<?php xin_the_icon( 'settings' ); ?><?php esc_html_e( 'Админка', 'xin-com' ); ?>
				</a>
			</nav>
...
case 'new-novel':
					case 'edit-novel':
						...
						if ( $xin_novel && ! xin_owns( $xin_novel->ID ) ) {
							echo '<p class="xin-empty-inline">' . esc_html__( 'Это не ваш проект.', 'xin-com' ) . '</p>';
							break;
						}
...
case 'new-chapter':
					case 'edit-chapter':
						...
						if ( $xin_chapter && ! xin_owns( $xin_chapter->ID ) ) {
							echo '<p class="xin-empty-inline">' . esc_html__( 'Это не ваша глава.', 'xin-com' ) . '</p>';
							break;
						}
