...
function xin_ago( $timestamp ) {
	...
	if ( $diff < 60 ) {
		return __( 'только что', 'xin-com' );
	}

	$units = array(
		array( YEAR_IN_SECONDS, __( 'год', 'xin-com' ), __( 'года', 'xin-com' ), __( 'лет', 'xin-com' ) ),
		array( MONTH_IN_SECONDS, __( 'месяц', 'xin-com' ), __( 'месяца', 'xin-com' ), __( 'месяцев', 'xin-com' ) ),
		array( DAY_IN_SECONDS, __( 'день', 'xin-com' ), __( 'дня', 'xin-com' ), __( 'дней', 'xin-com' ) ),
		array( HOUR_IN_SECONDS, __( 'час', 'xin-com' ), __( 'часа', 'xin-com' ), __( 'часов', 'xin-com' ) ),
		array( MINUTE_IN_SECONDS, __( 'минуту', 'xin-com' ), __( 'минуты', 'xin-com' ), __( 'минут', 'xin-com' ) ),
	);
	...
	foreach ( $units as $unit ) {
		if ( $diff >= $unit[0] ) {
			...
			return sprintf( __( '%1$d %2$s назад', 'xin-com' ), $n, xin_plural( $n, $unit[1], $unit[2], $unit[3] ) );
		}
	}
...
function xin_fav_button( $novel_id, $inline = false ) {
	...
	printf(
		...
		esc_attr__( 'В библиотеку', 'xin-com' ),
		esc_attr__( 'В библиотеку', 'xin-com' ),
		xin_icon( 'bookmark' ), 
		$inline ? '<span>' . esc_html__( 'В библиотеку', 'xin-com' ) . '</span>' : ''
	);
}
	...
	<a class="xin-chapcard" href="<?php echo esc_url( get_permalink( $chapter_id ) ); ?>">
		...
		<span class="xin-chapcard__body">
			...
			<span class="xin-chapcard__title">
				<?php if ( $label ) : ?>
					<span class="xin-chapcard__num"><?php printf( esc_html__( 'Гл. %s', 'xin-com' ), esc_html( $label ) ); ?></span>
				<?php endif; ?>
...
function xin_breadcrumbs() {
	...
	$home = '<a href="' . esc_url( home_url( '/' ) ) . '">' . esc_html__( 'Главная', 'xin-com' ) . '</a>';
	$out  = array( $home );

	if ( is_singular( 'chapter' ) ) {
		$novel_id = xin_chapter_novel_id( get_the_ID() );
		$out[]    = '<a href="' . esc_url( get_post_type_archive_link( 'novel' ) ) . '">' . esc_html__( 'Каталог', 'xin-com' ) . '</a>';
		if ( $novel_id ) {
		...
		$out[] = '<span>' . esc_html__( 'Каталог', 'xin-com' ) . '</span>';
	} elseif ( is_search() ) {
		$out[] = '<span>' . esc_html__( 'Поиск', 'xin-com' ) . '</span>';
	} elseif ( is_page() ) {
		$out[] = '<span>' . esc_html( get_the_title() ) . '</span>';
	}
