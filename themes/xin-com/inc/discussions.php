...
function xin_talk_must_log_in( $data ) {
	if ( ! is_user_logged_in() ) {
		wp_die( esc_html__( 'Чтобы участвовать в обсуждении, войдите в аккаунт.', 'xin-com' ), '', array( 'back_link' => true ) );
	}
...
function xin_talk_like( $request ) {
	...
	if ( ! $comment ) {
		return new WP_Error( 'xin_no_comment', __( 'Комментарий не найден.', 'xin-com' ), array( 'status' => 404 ) );
	}
				...
				<div class="xin-talk__tools">
					...
					<?php if ( ! $depth && is_user_logged_in() ) : ?>
						<button type="button" class="xin-talk__reply" data-xin-reply="<?php echo (int) $comment->comment_ID; ?>" data-name="<?php echo esc_attr( $comment->comment_author ); ?>">
							<?php esc_html_e( 'Ответить', 'xin-com' ); ?>
						</button>
					<?php endif; ?>
				</div>
...
function xin_talk_render( $post_id = 0 ) {
	...
	<section class="xin-talk" id="xin-talk">
		<div class="xin-talk__head-row">
			<h2><?php esc_html_e( 'Обсуждение', 'xin-com' ); ?></h2>
			<span><?php echo esc_html( sprintf( xin_plural( $total, __( '%d сообщение', 'xin-com' ), __( '%d сообщения', 'xin-com' ), __( '%d сообщений', 'xin-com' ) ), $total ) ); ?></span>
		</div>
		...
		<?php if ( is_user_logged_in() ) : ?>
			<form class="xin-talk__form" method="post" action="<?php echo esc_url( site_url( '/wp-comments-post.php' ) ); ?>" data-xin-talk-form>
				<div class="xin-talk__reply-to" data-xin-reply-note hidden>
					<span></span>
					<button type="button" data-xin-reply-cancel><?php esc_html_e( 'отменить', 'xin-com' ); ?></button>
				</div>
				<textarea name="comment" rows="3" required placeholder="<?php esc_attr_e( 'Что скажете? ||так прячется спойлер||', 'xin-com' ); ?>"></textarea>
				<div class="xin-talk__formfoot">
					<small><?php esc_html_e( '**жирный**, _курсив_, ||спойлер||', 'xin-com' ); ?></small>
					<button class="btn btn-primary btn-sm" type="submit"><?php esc_html_e( 'Отправить', 'xin-com' ); ?></button>
				</div>
		...
		<?php else : ?>
			<p class="xin-talk__guest">
				<?php esc_html_e( 'Обсуждение открыто для тех, кто вошёл в аккаунт.', 'xin-com' ); ?>
				<a href="<?php echo esc_url( xin_login_url( get_permalink( $post_id ) ) ); ?>"><?php esc_html_e( 'Войти', 'xin-com' ); ?></a>
			</p>
		<?php endif; ?>
		...
		<?php else : ?>
			<p class="xin-talk__empty"><?php esc_html_e( 'Пока тихо. Скажите первое слово.', 'xin-com' ); ?></p>
		<?php endif; ?>
	</section>
	<?php
}
