...
class XIN_Widget_Novels extends WP_Widget {
	public function __construct() {
		parent::__construct(
			'xin_novels',
			__( 'XI: подборка новелл', 'xin-com' ),
			array( 'description' => __( 'Топ по просмотрам, оценке, новинки или обновления.', 'xin-com' ) )
		);
	}

	public function widget( $args, $instance ) {
		$title = ! empty( $instance['title'] ) ? $instance['title'] : __( 'Популярное', 'xin-com' );
		...
		foreach ( $ids as $id ) {
			...
			<a class="xin-widget-novel" href="<?php echo esc_url( get_permalink( $id ) ); ?>">
				...
				<span>
					...
					<small>
						<span><?php echo esc_html( xin_num( xin_get_views( $id ) ) ); ?> <?php esc_html_e( 'просм.', 'xin-com' ); ?></span>
						<?php if ( $rating['count'] ) : ?>
	...
	public function form( $instance ) {
		$title = isset( $instance['title'] ) ? $instance['title'] : __( 'Популярное', 'xin-com' );
		$type  = isset( $instance['type'] ) ? $instance['type'] : 'popular';
		$count = isset( $instance['count'] ) ? (int) $instance['count'] : 5;
		?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Заголовок', 'xin-com' ); ?></label>
			<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>">
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'type' ) ); ?>"><?php esc_html_e( 'Что показывать', 'xin-com' ); ?></label>
			<select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'type' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'type' ) ); ?>">
				<?php
				$types = array(
					'popular' => __( 'По просмотрам', 'xin-com' ),
					'rating'  => __( 'По оценке', 'xin-com' ),
					'latest'  => __( 'Новинки', 'xin-com' ),
					'updated' => __( 'Недавно обновлены', 'xin-com' ),
				);
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'count' ) ); ?>"><?php esc_html_e( 'Сколько', 'xin-com' ); ?></label>
...
class XIN_Widget_Chapters extends WP_Widget {
	public function __construct() {
		parent::__construct(
			'xin_chapters',
			__( 'XI: последние главы', 'xin-com' ),
			array( 'description' => __( 'Свежие публикации со всего сайта.', 'xin-com' ) )
		);
	}

	public function widget( $args, $instance ) {
		$title = ! empty( $instance['title'] ) ? $instance['title'] : __( 'Последние главы', 'xin-com' );
		$count = ! empty( $instance['count'] ) ? (int) $instance['count'] : 5;
		...
		foreach ( $ids as $id ) {
			...
			<a class="xin-widget-novel" href="<?php echo esc_url( get_permalink( $id ) ); ?>">
				...
				<span>
					<h4><?php echo $label ? esc_html( sprintf( __( 'Гл. %s — ', 'xin-com' ), $label ) ) : ''; ?><?php echo esc_html( get_the_title( $id ) ); ?></h4>
	...
	public function form( $instance ) {
		$title = isset( $instance['title'] ) ? $instance['title'] : __( 'Последние главы', 'xin-com' );
		$count = isset( $instance['count'] ) ? (int) $instance['count'] : 5;
		?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Заголовок', 'xin-com' ); ?></label>
