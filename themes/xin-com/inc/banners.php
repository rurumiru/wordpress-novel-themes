...
function xin_register_banner_type() {
	register_post_type( 'xin_banner', array(
		'labels'          => array(
			'name'               => __( 'Баннеры', 'xin-com' ),
			'singular_name'      => __( 'Баннер', 'xin-com' ),
			'add_new'            => __( 'Добавить', 'xin-com' ),
			'add_new_item'       => __( 'Добавить баннер', 'xin-com' ),
			'edit_item'          => __( 'Редактировать баннер', 'xin-com' ),
			'all_items'          => __( 'Баннеры', 'xin-com' ),
			'not_found'          => __( 'Баннеров пока нет', 'xin-com' ),
			'menu_name'          => __( 'Баннеры', 'xin-com' ),
			'featured_image'     => __( 'Картинка баннера', 'xin-com' ),
			'set_featured_image' => __( 'Задать картинку', 'xin-com' ),
		),
		'public'          => false,
		'show_ui'         => true,
		'show_in_menu'    => true,
...
function xin_banner_meta_box() {
	add_meta_box( 'xin-banner', __( 'Содержимое баннера', 'xin-com' ), 'xin_banner_box', 'xin_banner', 'normal', 'high' );
}
...
function xin_banner_box( $post ) {
	...
	<div class="xin-bfields">
		<p class="xin-full">
			<label for="xin_b_subtitle"><?php esc_html_e( 'Подзаголовок', 'xin-com' ); ?></label>
			<input type="text" id="xin_b_subtitle" name="xin_b_subtitle" value="<?php echo esc_attr( $subtitle ); ?>">
		</p>
		<p class="xin-full">
			<label for="xin_b_text"><?php esc_html_e( 'Текст', 'xin-com' ); ?></label>
			<textarea id="xin_b_text" name="xin_b_text" rows="2"><?php echo esc_textarea( $text ); ?></textarea>
		</p>
		<p>
			<label for="xin_b_link"><?php esc_html_e( 'Ссылка', 'xin-com' ); ?></label>
			<input type="url" id="xin_b_link" name="xin_b_link" value="<?php echo esc_attr( $link ); ?>" placeholder="https://">
		</p>
		<p>
			<label for="xin_b_cta"><?php esc_html_e( 'Надпись на кнопке', 'xin-com' ); ?></label>
			<input type="text" id="xin_b_cta" name="xin_b_cta" value="<?php echo esc_attr( $cta ); ?>" placeholder="<?php esc_attr_e( 'Открыть', 'xin-com' ); ?>">
		</p>
		<p>
			<label for="xin_b_badge"><?php esc_html_e( 'Бейдж', 'xin-com' ); ?></label>
			<input type="text" id="xin_b_badge" name="xin_b_badge" value="<?php echo esc_attr( $badge ); ?>" placeholder="<?php esc_attr_e( 'Новинка', 'xin-com' ); ?>">
		</p>
		<p>
			<label for="xin_b_align"><?php esc_html_e( 'Положение текста', 'xin-com' ); ?></label>
			<select id="xin_b_align" name="xin_b_align">
				<option value="left" <?php selected( $align, 'left' ); ?>><?php esc_html_e( 'Слева', 'xin-com' ); ?></option>
				<option value="center" <?php selected( $align, 'center' ); ?>><?php esc_html_e( 'По центру', 'xin-com' ); ?></option>
				<option value="right" <?php selected( $align, 'right' ); ?>><?php esc_html_e( 'Справа', 'xin-com' ); ?></option>
			</select>
		</p>
		<div class="xin-full">
			<label><?php esc_html_e( 'Картинка для телефона', 'xin-com' ); ?></label>
			...
			<button type="button" class="button" id="xin-bmobile-pick"><?php esc_html_e( 'Выбрать', 'xin-com' ); ?></button>
			<button type="button" class="button-link" id="xin-bmobile-clear" style="margin-left:8px;color:#b32d2e"><?php esc_html_e( 'Убрать', 'xin-com' ); ?></button>
			<p class="description"><?php esc_html_e( 'Необязательно. Если не задана, на телефоне используется основная картинка.', 'xin-com' ); ?></p>
		</div>
		<p class="xin-full description">
			<?php esc_html_e( 'Основная картинка — «Изображение записи» справа. Оптимальный размер 1920×720. Порядок показа задаётся полем «Порядок» в блоке «Атрибуты страницы».', 'xin-com' ); ?>
		</p>
	</div>
	<script>
	jQuery(function ($) {
		...
		$('#xin-bmobile-pick').on('click', function (e) {
			...
			frame = wp.media({ title: '<?php echo esc_js( __( 'Картинка для телефона', 'xin-com' ) ); ?>', multiple: false });
...
function xin_banner_columns( $columns ) {
	...
	foreach ( $columns as $key => $label ) {
		if ( 'title' === $key ) {
			$new['xin_b_image'] = __( 'Картинка', 'xin-com' );
		}
		$new[ $key ] = $label;
		if ( 'title' === $key ) {
			$new['xin_b_order'] = __( 'Порядок', 'xin-com' );
		}
	}
	return $new;
}
