...
$xin_tabs = array(
	'novels'   => __( 'Проекты', 'xin-com' ),
	'chapters' => __( 'Главы', 'xin-com' ),
	'posts'    => __( 'Статьи', 'xin-com' ),
);
?>
		...
		<div>
			<h1 class="xin-profile__name">
				...
				<?php if ( $xin_stats['novels'] > 0 ) : ?>
					<span class="xin-badge xin-badge--primary"><?php xin_the_icon( 'pen' ); ?><?php esc_html_e( 'автор', 'xin-com' ); ?></span>
				<?php endif; ?>
			</h1>
			...
			<p class="xin-muted" style="margin:0">
				<?php
				printf(
					esc_html__( 'На площадке с %s', 'xin-com' ),
			...
			<div class="xin-profile__stats">
				<div><b><?php echo esc_html( number_format_i18n( $xin_stats['novels'] ) ); ?></b><span><?php esc_html_e( 'проектов', 'xin-com' ); ?></span></div>
				<div><b><?php echo esc_html( number_format_i18n( $xin_stats['chapters'] ) ); ?></b><span><?php esc_html_e( 'глав', 'xin-com' ); ?></span></div>
				<div><b><?php echo esc_html( xin_num( $xin_stats['views'] ) ); ?></b><span><?php esc_html_e( 'прочтений', 'xin-com' ); ?></span></div>
				<div><b><?php echo esc_html( number_format_i18n( $xin_stats['posts'] ) ); ?></b><span><?php esc_html_e( 'статей', 'xin-com' ); ?></span></div>
			</div>
				...
				<p class="xin-profile__streak">
					...
					<span>
						...
						<?php if ( $xin_reading['read'] > 0 ) : ?>
							· <?php echo esc_html( sprintf( xin_plural( $xin_reading['read'], __( '%d глава прочитана', 'xin-com' ), __( '%d главы прочитано', 'xin-com' ), __( '%d глав прочитано', 'xin-com' ) ), $xin_reading['read'] ) ); ?>
						<?php endif; ?>
					</span>
				</p>
			<?php endif; ?>
			...
			<div class="xin-profile__actions">
				<?php if ( $xin_self ) : ?>
					<a class="btn btn-primary" href="<?php echo esc_url( xin_dashboard_url() ); ?>">
						<?php xin_the_icon( 'pen' ); ?><?php esc_html_e( 'Кабинет автора', 'xin-com' ); ?>
					</a>
					<a class="btn btn-outline" href="<?php echo esc_url( admin_url( 'profile.php' ) ); ?>">
						<?php xin_the_icon( 'settings' ); ?><?php esc_html_e( 'Настройки профиля', 'xin-com' ); ?>
					</a>
				<?php else : ?>
					<a class="btn btn-outline" href="<?php echo esc_url( get_author_feed_link( $xin_id ) ); ?>">
						<?php xin_the_icon( 'rss' ); ?><?php esc_html_e( 'Следить за обновлениями', 'xin-com' ); ?>
					</a>
				<?php endif; ?>

				<?php if ( $xin_don ) : ?>
					<a class="btn btn-gold" href="<?php echo esc_url( $xin_don ); ?>" target="_blank" rel="noopener nofollow">
						<?php xin_the_icon( 'gift' ); ?><?php esc_html_e( 'Поддержать', 'xin-com' ); ?>
					</a>
				<?php endif; ?>
				...
				<?php if ( $xin_links ) : ?>
					<span class="xin-profile__links">
						<?php foreach ( $xin_links as $xin_link ) : ?>
							<a class="btn btn-icon" href="<?php echo esc_url( $xin_link['url'] ); ?>" target="_blank" rel="noopener nofollow" aria-label="<?php esc_attr_e( 'Ссылка автора', 'xin-com' ); ?>">
...
<div class="xin-wrap xin-section">
	...
	if ( 'chapters' === $xin_tab ) {
		...
		if ( $xin_items ) {
			...
			echo '<p class="xin-empty-inline">' . esc_html__( 'Опубликованных глав пока нет.', 'xin-com' ) . '</p>';
		}
		...
		if ( $xin_items ) {
			...
			echo '<p class="xin-empty-inline">' . esc_html__( 'Статей пока нет.', 'xin-com' ) . '</p>';
		}
	} else {
		...
		if ( $xin_items ) {
			...
			<div class="xin-empty">
				...
				<h2><?php esc_html_e( 'Здесь появятся ваши проекты', 'xin-com' ); ?></h2>
				<p><?php esc_html_e( 'Заведите первый тайтл — на это уходит пара минут, а страница профиля соберётся сама.', 'xin-com' ); ?></p>
				<a class="btn btn-primary xin-mt-2" href="<?php echo esc_url( xin_dashboard_url( array( 'view' => 'new-novel' ) ) ); ?>">
					<?php xin_the_icon( 'plus' ); ?><?php esc_html_e( 'Новый проект', 'xin-com' ); ?>
				</a>
			</div>
			<?php
		} else {
			echo '<p class="xin-empty-inline">' . esc_html__( 'Проектов пока нет.', 'xin-com' ) . '</p>';
		}
	}
	?>
...
if ( $xin_open ) :
	...
	<div class="xin-wrap">
		<section class="xin-awards">
			<h2 class="xin-awards__title"><?php esc_html_e( 'Достижения', 'xin-com' ); ?></h2>
