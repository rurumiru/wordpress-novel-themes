...
function xin_push_enqueue_assets() {
	...
	wp_localize_script(
		...
		array(
			...
			'i18n'           => array(
				'enabled'       => __( 'Уведомления вкл.', 'xi-novels' ),
				'disabled'      => __( 'Уведомлять', 'xi-novels' ),
				'unsupported'   => __( 'Нужны HTTPS и современный браузер', 'xi-novels' ),
				'denied'        => __( 'Заблокировано в браузере', 'xi-novels' ),
				'error'         => __( 'Не получилось обновить уведомления. Попробуйте ещё раз.', 'xi-novels' ),
				'subscribing'   => __( 'Включаю…', 'xi-novels' ),
				'unsubscribing' => __( 'Выключаю…', 'xi-novels' ),
			),
		)
	);
}
...
function xin_push_enforce_rate_limit( $scope, $limit, $window ) {
	if ( ! xin_push_rate_limit_ok( $scope, $limit, $window ) ) {
		wp_send_json_error(
			array(
				'message' => __( 'Слишком много запросов. Подождите и попробуйте снова.', 'xi-novels' ),
				'code'    => 'rate_limited',
			),
			429
		);
...
function xin_ajax_push_subscribe() {
	...
	if (
		...
		wp_send_json_error( array( 'message' => __( 'Неверные данные подписки.', 'xi-novels' ) ), 400 );
	}
	...
	if ( $existing_id > 0 ) {
		...
		if ( false === $ok ) {
			wp_send_json_error( array( 'message' => __( 'Не получилось обновить подписку.', 'xi-novels' ) ), 500 );
		}
	} else {
		$ok = $wpdb->insert(
			$table,
		...
		if ( ! $ok ) {
			...
			if ( ! $ok2 ) {
				wp_send_json_error(
					array( 'message' => __( 'Не получилось сохранить подписку.', 'xi-novels' ) ),
					500
				);
			}
		}
...
function xin_ajax_push_unsubscribe() {
	...
	$endpoint = isset( $_POST['endpoint'] ) ? xin_push_sanitize_endpoint( wp_unslash( $_POST['endpoint'] ) ) : '';
	if ( ! $endpoint ) {
		wp_send_json_error( array( 'message' => __( 'Неверный endpoint.', 'xi-novels' ) ), 400 );
	}
...
function xin_push_verify_ajax_request( $novel_id, $scope = 'request', $limit = 60, $window = HOUR_IN_SECONDS ) {
	if ( ! $novel_id || get_post_type( $novel_id ) !== 'novel' || get_post_status( $novel_id ) !== 'publish' ) {
		wp_send_json_error( array( 'message' => __( 'Неверный тайтл.', 'xi-novels' ), 'code' => 'invalid_novel' ), 400 );
	}
	...
	if ( ! $nonce || ! wp_verify_nonce( $nonce, 'xin_push_' . $novel_id ) ) {
		wp_send_json_error(
			array(
				'message' => __( 'Сессия истекла — повторяю…', 'xi-novels' ),
				'code'    => 'bad_nonce',
			),
			403
		);
...
function xin_push_build_payload( $novel_id, $chapter_ids ) {
	...
	$title = ( $count === 1 )
		? __( 'Доступна 1 новая глава', 'xi-novels' )
		: sprintf(
			/* translators: %d: number of new free chapters posted in this batch */
			__( 'Опубликовано %d новых глав', 'xi-novels' ),
			$count
		);
	...
	return array(
		...
		'body'  => $novel_title
			? sprintf( __( '%s — вышли бесплатные главы', 'xi-novels' ), $novel_title )
			: __( 'Вышли бесплатные главы', 'xi-novels' ),
		'url'   => $url ? $url : home_url( '/' ),
		'icon'  => $icon,
...
function xin_push_render_bell_button( $novel_id ) {
	...
	<button
		...
		aria-pressed="false"
		aria-label="<?php echo esc_attr__( 'Уведомления о главах', 'xi-novels' ); ?>"
		title="<?php echo esc_attr__( 'Браузерные уведомления, когда выходят бесплатные главы', 'xi-novels' ); ?>"
	>
		<span class="xin-push-bell-icon"><?php xin_the_icon( 'bell' ); ?></span>
		<span class="xin-push-bell-label"><?php esc_html_e( 'Уведомлять', 'xi-novels' ); ?></span>
	</button>
	<?php
}
