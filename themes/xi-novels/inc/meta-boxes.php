...
function xin_add_meta_boxes() {
	add_meta_box( 'xin-novel', __( 'Карточка тайтла', 'xi-novels' ), 'xin_novel_box', 'novel', 'normal', 'high' );
	add_meta_box( 'xin-novel-art', __( 'Фон-арт (широкий)', 'xi-novels' ), 'xin_novel_art_box', 'novel', 'side', 'low' );
	add_meta_box( 'xin-chapter', __( 'Параметры главы', 'xi-novels' ), 'xin_chapter_box', 'chapter', 'side', 'high' );
}
...
function xin_novel_box( $post ) {
	...
	<div class="xin-fields">
		<p>
			<label for="xin_author_name"><?php esc_html_e( 'Автор оригинала', 'xi-novels' ); ?></label>
			<input type="text" id="xin_author_name" name="xin_author_name" value="<?php echo esc_attr( $author ); ?>" placeholder="<?php esc_attr_e( 'Имя автора', 'xi-novels' ); ?>">
		</p>
		<p>
			<label for="xin_original_title"><?php esc_html_e( 'Оригинальное название', 'xi-novels' ); ?></label>
		...
		<p class="xin-full">
			<label for="xin_source"><?php esc_html_e( 'Ссылка на первоисточник', 'xi-novels' ); ?></label>
			<input type="url" id="xin_source" name="xin_source" value="<?php echo esc_attr( $source ); ?>" placeholder="https://">
		</p>
		<p>
			<label for="xin_rating"><?php esc_html_e( 'Оценка (0–5)', 'xi-novels' ); ?></label>
			<input type="number" step="0.1" min="0" max="5" id="xin_rating" name="xin_rating" value="<?php echo esc_attr( $rating ); ?>">
			<span class="description"><?php printf( esc_html__( 'Голосов: %d', 'xi-novels' ), (int) $rcount ); ?></span>
		</p>
		<p>
			<label for="xin_views"><?php esc_html_e( 'Просмотры', 'xi-novels' ); ?></label>
			<input type="number" id="xin_views" name="xin_views" value="<?php echo esc_attr( $views ); ?>" min="0">
			<span class="description"><?php esc_html_e( 'Счётчик растёт сам; поле — для переноса статистики.', 'xi-novels' ); ?></span>
		</p>
		<p class="xin-full">
			<label><input type="checkbox" name="xin_adult" value="1" <?php checked( $adult ); ?>> <?php esc_html_e( 'Материал 18+ (обложка размывается в каталоге)', 'xi-novels' ); ?></label>
			<label><input type="checkbox" name="xin_featured" value="1" <?php checked( $featured ); ?>> <?php esc_html_e( 'Выбор редакции (показывать в витрине главной)', 'xi-novels' ); ?></label>
		</p>
	</div>
	<?php
}
...
function xin_novel_art_box( $post ) {
	...
	<div class="xin-art">
		...
		<button type="button" class="button" id="xin-art-pick"><?php esc_html_e( 'Выбрать арт', 'xi-novels' ); ?></button>
		<button type="button" class="button-link" id="xin-art-clear" style="margin-left:8px;color:#b32d2e"><?php esc_html_e( 'Убрать', 'xi-novels' ); ?></button>
		<p class="description"><?php esc_html_e( 'Широкая картинка для hero-блока и плиток «самые любимые».', 'xi-novels' ); ?></p>
	</div>
	<script>
	jQuery(function ($) {
		var frame;
		$('#xin-art-pick').on('click', function (e) {
			...
			frame = wp.media({ title: '<?php echo esc_js( __( 'Фон-арт тайтла', 'xi-novels' ) ); ?>', multiple: false });
...
function xin_chapter_box( $post ) {
	...
	?>
	<p>
		<label for="xin_novel"><strong><?php esc_html_e( 'Новелла', 'xi-novels' ); ?></strong></label>
		<select id="xin_novel" name="xin_novel" style="width:100%">
			<option value="">— <?php esc_html_e( 'не выбрана', 'xi-novels' ); ?> —</option>
	...
	<p>
		<label for="xin_number"><strong><?php esc_html_e( 'Номер главы', 'xi-novels' ); ?></strong></label>
...
function xin_novel_columns( $columns ) {
	...
	foreach ( $columns as $key => $label ) {
		if ( 'title' === $key ) {
			$new['xin_cover'] = __( 'Обложка', 'xi-novels' );
		}
		$new[ $key ] = $label;
		if ( 'title' === $key ) {
			$new['xin_chapters'] = __( 'Главы', 'xi-novels' );
			$new['xin_views']    = __( 'Просмотры', 'xi-novels' );
			$new['xin_rating']   = __( 'Оценка', 'xi-novels' );
		}
	}
	return $new;
}
...
function xin_chapter_columns( $columns ) {
	...
	foreach ( $columns as $key => $label ) {
		$new[ $key ] = $label;
		if ( 'title' === $key ) {
			$new['xin_novel']  = __( 'Новелла', 'xi-novels' );
			$new['xin_number'] = __( '№', 'xi-novels' );
		}
	}
	return $new;
}
...
function xin_chapter_column( $column, $post_id ) {
	if ( 'xin_novel' === $column ) {
		...
		echo $novel_id
			...
			: '<em>' . esc_html__( 'не привязана', 'xi-novels' ) . '</em>';
	}
...
function xin_chapter_filter() {
	...
	echo '<select name="xin_novel_filter"><option value="">' . esc_html__( 'Все новеллы', 'xi-novels' ) . '</option>';
