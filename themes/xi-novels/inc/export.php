...
function xin_download_audiences() {
	return array(
		'all'       => __( 'Всем, включая гостей', 'xi-novels' ),
		'members'   => __( 'Любому вошедшему', 'xi-novels' ),
		'plus'      => __( 'Только с доступом PLUS', 'xi-novels' ),
		'plus_role' => __( 'PLUS или выбранные роли', 'xi-novels' ),
		'roles'     => __( 'Только выбранным ролям', 'xi-novels' ),
	);
}
...
function xin_download_denied() {
	...
	if ( ! is_user_logged_in() ) {
		return array(
			'text' => __( 'Скачивание книг доступно не всем посетителям. Войдите в аккаунт.', 'xi-novels' ),
			'url'  => xin_login_url( get_permalink() ),
			'link' => __( 'Войти', 'xi-novels' ),
		);
	}

	if ( 'roles' === $mode ) {
		$text = __( 'Скачивание книг открыто только отдельным ролям площадки.', 'xi-novels' );
	} elseif ( 'plus_role' === $mode ) {
		$text = __( 'Скачивание книг открыто по доступу PLUS и отдельным ролям площадки.', 'xi-novels' );
	} else {
		$text = __( 'Скачивание книг входит в доступ PLUS.', 'xi-novels' );
	}
