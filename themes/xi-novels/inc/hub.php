...
function xin_hub_ago( $stamp ) {
	...
	if ( $diff < MINUTE_IN_SECONDS ) {
		...
		$span = sprintf( __( '%s сек.', 'xi-novels' ), number_format_i18n( max( 1, $diff ) ) );
	} elseif ( $diff < HOUR_IN_SECONDS ) {
		/* translators: %s: number of minutes. */
		$span = sprintf( __( '%s мин.', 'xi-novels' ), number_format_i18n( (int) floor( $diff / MINUTE_IN_SECONDS ) ) );
	} elseif ( $diff < DAY_IN_SECONDS ) {
		/* translators: %s: number of hours. */
		$span = sprintf( __( '%s ч.', 'xi-novels' ), number_format_i18n( (int) floor( $diff / HOUR_IN_SECONDS ) ) );
	} else {
		/* translators: %s: number of days. */
		$span = sprintf( __( '%s дн.', 'xi-novels' ), number_format_i18n( (int) floor( $diff / DAY_IN_SECONDS ) ) );
	}

	/* translators: %s: time span, already formatted. */
	return sprintf( __( '%s назад', 'xi-novels' ), $span );
}
