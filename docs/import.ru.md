...
## 3. Главы из текстовых файлов или ZIP-архива

Чаще всего у переводчика на руках именно папка пронумерованных файлов. Импортёр принимает её напрямую — манифест не нужен.

```bash
# папка файлов в один тайтл
php tools/import-novels.php --wp=/var/www/site --from-dir=./chapters --novel="Печать девятого неба"

# то же самое архивом
php tools/import-novels.php --wp=/var/www/site --from-zip=chapters.zip --novel-slug=pechat-devyatogo

# дописать в существующий тайтл, пометив всё с 40-й главы как PLUS
php tools/import-novels.php --wp=/var/www/site --from-zip=vol3.zip --novel-id=412 --locked-from=40
```

| Опция | Смысл |
|---|---|
| `--from-dir=PATH` | Папка с файлами глав |
| `--from-zip=PATH` | Архив с тем же. Единственная обёртывающая папка внутри разворачивается сама |
| `--novel=NAME` | Тайтл-получатель. Создаётся, если ещё нет |
| `--novel-slug=SLUG` | Тайтл по слагу |
| `--novel-id=ID` | Тайтл по ID записи |
| `--start=N` | С какого номера считать, если в именах файлов номеров нет. По умолчанию `1` |
| `--locked-from=N` | Пометить главы начиная с этого номера как PLUS |
| `--encoding=ENC` | Принудительная кодировка источника, например `windows-1251` |
| `--status`, `--author`, `--dry-run` | Как в режиме манифеста |

### Какие файлы понимает

| Расширение | Как обрабатывается |
|---|---|
| `.txt` | Обычный текст. Пустая строка начинает новый абзац, одиночный перенос внутри абзаца становится `<br>` — так обычно набирают диалоги |
| `.html`, `.htm` | Берётся как есть. Если это целый документ, используется только содержимое `<body>` |
| `.md` | Заголовки (`#`, `##`, `###`), `**жирный**` и `*курсив*` конвертируются, остальное становится абзацами |

Файлы с другими расширениями игнорируются, поэтому обложки и заметки в той же папке ничему не мешают.

### Имя файла даёт номер и название

| Имя файла | Номер | Название главы |
|---|---|---|
| `001. Осколок.txt` | 1 | Осколок |
| `002 - Первый снег.txt` | 2 | Первый снег |
| `012.5_Экстра.html` | 12.5 | Экстра |
| `Глава 3. Долг.md` | 3 | Долг |
| `Chapter 4 - Night guest.txt` | 4 | Night guest |
| `Осколок.txt` | по порядку файлов | Осколок |

Файлы сортируются по-человечески: `2` идёт раньше `10`. Если номера в имени нет, берётся позиция файла в этом порядке, начиная с `--start`.

### Один архив — несколько тайтлов

Если в архиве или папке лежат **подпапки**, а `--novel` не задан, каждая подпапка становится отдельным тайтлом с именем папки:

```
collection.zip
├── Печать девятого неба/
│   ├── 001. Осколок.txt
│   └── 002. Первый снег.txt
└── Полночная кондитерская/
    ├── 001. Часы работы.txt
    └── 002. Постоянный гость.txt
```
...
Существующие тайтлы находятся по слагу и переиспользуются, недостающие создаются пустыми — обложку и описание потом добавите в кабинете.

### Кодировки

Файлы читаются как UTF-8, метка порядка байтов срезается. Если содержимое не проходит проверку UTF-8, оно переводится из `windows-1251` — обычный случай для старых выгрузок из Word и «Блокнота». Любую другую кодировку можно задать через `--encoding=`, например `--encoding=koi8-r`.

### Повторный запуск

Главы сопоставляются по номеру внутри тайтла, поэтому повторный импорт того же архива обновляет существующие главы, а не плодит копии. Поправили опечатку в файле, запустили команду снова — изменится только эта глава.

Архив распаковывается во временную папку и удаляется после работы; записи, ведущие за пределы архива, и мусор архиваторов (`__MACOSX`, `.DS_Store`) пропускаются.
...
## 6. Настройки сервера под тяжёлые файлы

Обложки, широкие арты и файлы импорта упираются сразу в пять ограничений. Поднимать нужно все — одного заниженного достаточно, чтобы загрузка не прошла.
...
| Импорт останавливается на полпути, страница пустая | Не хватило памяти или истекло время запроса | Запускать импортёр из PHP CLI, поднять `memory_limit` |
