...
<h3>Лучшая тема WordPress для новелл. И вот чем это подтверждается.</h3>

<p>
Каталог, страница тайтла, полноэкранная читалка с бесконечной прокруткой,<br>
озвучкой и словарём проекта, рейтинги, кабинет автора прямо на сайте,<br>
библиотека читателя, выгрузка в EPUB и FB2 — <b>всё это уже в теме</b>.<br>
Ни одного обязательного плагина, ни одной внешней загрузки, ни одной сборки.<br>
...
[![История версий](https://img.shields.io/badge/%D0%98%D1%81%D1%82%D0%BE%D1%80%D0%B8%D1%8F_%D0%B2%D0%B5%D1%80%D1%81%D0%B8%D0%B9-%D0%B1%D0%B5%D1%82%D0%B0_0.9.0-6366f1?style=for-the-badge)](CHANGELOG.md)

<br>

![Версия](https://img.shields.io/badge/%D0%B2%D0%B5%D1%80%D1%81%D0%B8%D1%8F-%D0%B1%D0%B5%D1%82%D0%B0_0.9.0-f59e0b?style=flat-square)
...
> [!NOTE]
> **Бета 0.9.0 — платформа работает целиком.** Её можно поставить сегодня и публиковать тайтлы и главы. Не финален **слой представления**: Bootstrap 5 — текущая база, а не конечная точка.

## Почему именно она

Громкие слова стоят ровно столько, сколько под ними цифр и кода. Вот они.

| | |
|:--|:--|
| 📚 **Всё в теме, а не в пяти плагинах** | Каталог, главы, читалка, рейтинги, оценки, библиотека, кабинет автора, выгрузка книг, обсуждения, словарь проекта, озвучка — один код, одна модель данных, один набор настроек. Три плагина в комплекте нужны только для массового импорта, массового редактирования и визуальной настройки облика |
| 🚀 **Держит тысячи глав** | Замерено на песочнице с **400 тайтлами и 14 000 глав**, среди которых тайтл на 4000 глав: его страница открывается за **0,52 с** с холодным кэшем, каталог — за 0,55 с, доска рейтинга — за 0,45 с. Оглавление грузится порциями, читалка держит окно глав вокруг текущей, число глав лежит в мете, а не пересчитывается на каждой карточке |
| 📖 **Читалка, ради которой возвращаются** | Бесконечная прокрутка: дочитал главу — следующая дописывается сама. Размер, ширина, интерлиньяж, шрифт и сорт бумаги; закладки и цитаты по абзацам, предложения правок автору, словарь имён, синтез речи, рельс управления слева и прогресс с оставшимся временем |
| 🔌 **Ноль внешних запросов** | Никаких CDN, шрифтов Google и счётчиков. Шрифты — системные, иконки — инлайновый SVG. Страница грузится с вашего домена целиком |
| 🛠 **Ноль сборки** | Нет npm, нет composer, нет шага компиляции. Скачали, распаковали в `wp-content/themes`, включили |
| ✍️ **Автор публикует с сайта** | Кабинет автора, редактор главы с автосохранением, расписание выхода, словарь проекта, соавторы — без захода в `/wp-admin` |
| 🌍 **Три языка из коробки** | 1179 строк в теме плюс 374 в плагинах: русский исходник, собранные `en_US` и `pt_BR`, переключатель RU / EN / PT в шапке |
| 🔍 **Исходник, который можно прочитать** | ~19 тысяч строк с комментариями на русском — не «зачем», а «почему именно так». Никакого обфусцированного JSON от конструктора |
...
<sub><code>/manage/</code> — пользователи, роли, очередь на проверку, тайтлы и настройки</sub>
...
| | |
|:--|:--|
| **0.9.0** | **Облик собран заново и платформа выдерживает большие каталоги.** Тёплая бумага вместо холодного графита, антиква в заголовках, волосяные линии вместо теней; страница тайтла — обложечный разворот с вкладками и колонкой карточек, читалка — панели по ширине сайта, рельс управления и **бесконечная прокрутка**: дочитал главу, следующая дописывается сама. Оглавление грузится порциями по 30, ритм выхода глав считается сам, «наугад» открывает случайный тайтл. Убран режим PLUS. Проверено на 400 тайтлах и 14 000 глав |
| **0.8.0** | **Тема стала XIN-Com, у неё появился раздел комиксов.** Новеллы и комиксы — два раздела с переключателем в шапке, у каждого своя главная, свой каталог, а у комиксов ещё и читалка страниц: лентой вниз или постранично, у манги справа налево. Страницы комиксов выгружаются в S3-совместимое хранилище, а в читалке выбирается сервер раздачи. Студия настраивает WordPress под тему: ссылки, служебные страницы, главная и меню |
...
| 📥 **EPUB и FB2** | Любой тайтл скачивается книгой — с обложкой, оглавлением и главами. Закрытые главы попадают в файл только тем, кому они открыты. Кому вообще доступна выгрузка — всем, вошедшим или выбранным ролям — решается в панели управления |
...
| 🔒 **Ранний доступ** | Глава помечается как закрытая: гостю недоступна, в оглавлении с замком, открывается команде проекта или после покупки |
...
| 🛠️ **Панель управления на сайте** | `/manage/`: пользователи и роли, очередь на проверку, все тайтлы и настройки площадки, включая доступ к скачиванию книг, — без `/wp-admin` |
...
| 🛒 **Платные главы** | Мост к WooCommerce: к главе привязывается товар, и она открывается после покупки |
...
| 🗂️ **Массовое управление тайтлами** | Плагин в комплекте: фильтры по владельцу, жанру, статусу, обложке и 18+, выделение с Shift — или всё найденное сразу, — а дальше публикация, жанры и метки, владелец и команда, одна обложка на пачку, ранний доступ на все главы тайтла, выгрузка в CSV и удаление. Каждый идентификатор ещё раз проверяется через `current_user_can()`, так что подменённая форма чужой тайтл не тронет |
...
**Панель управления** на `/manage/` — роли, очередь на проверку, тайтлы и настройки.
...
**Тёмная схема** — тёплый уголь, переключается из шапки.
...
1. **Внешний вид → Темы → XIN-Com → Активировать.** При активации тема сама создаёт страницы «Кабинет автора» и «Моя библиотека» и заводит статусы выпуска (Выходит / Завершён / Заморожен / Анонс).
...
| Бесконечная прокрутка глав | **Да** | Редко | Да |
| Каталог на тысячи глав | **Проверено: 14 000 глав** | Как повезёт | Да |
...
| Плагины в комплекте | **XI Studio**, **XIN-Com Import**, **XIN-Com Manager** — ~3,7 тыс. строк PHP | Каждый необязателен: тема работает и одна, плагины добавляют студию, массовый импорт и массовое управление |
...
```
themes/xin-com/      тема — всё описанное выше здесь
...
В теме есть готовая строка: **Настроить → подвал → «Работает на XIN-Com»**, по умолчанию выключена. Или своя ссылка:

```html
<a href="https://github.com/rurumiru/wordpress-novel-themes">Работает на теме XIN-Com</a>
```
