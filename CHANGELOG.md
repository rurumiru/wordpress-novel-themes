...
- **Главная и страница тайтла переписаны с нуля.** Правка старой разметки упёрлась в потолок: каждое исправление превращалось в переопределение, спорящее с pages.css, а страница всё равно читалась как приборная панель. Оба экрана собраны заново, со своей разметкой (приставки `hm-` и `nv-`), поэтому старые селекторы к ней просто не применяются и перекрикивать их больше не нужно. Правила обоих экранов лежат в `assets/css/screens.css`, а `look.css` оставил себе только общее — токены, типографику, кнопки, шапку сайта, меру строки в читалке и карточку каталога.

  Страница тайтла: полоса с обложкой в 236 пикселей, название, оценка в порядке эталона — звёзды, число, количество голосов, — строка фактов со значками, жанры-чипы и действия; ниже одна колонка в 1040 пикселей с двумя вкладками. Всё, что жило в боковой панели, стало разделами внутри первой вкладки: справка о тайтле полотном в две колонки, команда — овалами с аватарами, виджет оценки, шесть похожих тайтлов и, в конце, виджеты, которые владелец площадки положил в боковую колонку тайтла. Ничего настроенного не потерялось, и вторая колонка больше не отнимает ширину у текста.

  Главная: витрина с ведущим тайтлом, его описанием и четвёркой следующих рядом; три доски рейтингов в ряд, у первой тройки в каждой — обложки; полки новинок и недавно обновлённых; лента последних глав в две колонки с обложками и относительным временем; жанры-чипы и два призыва. Каждый блок по-прежнему слушается своего переключателя `xin_show()`, так что владелец площадки сохраняет власть над составом главной.

- **Тринадцать новых строк переведены** на английский и бразильский португальский.

## [0.7.1-beta] — 2026-08-27

### Changed

- **Страница тайтла собрана по эталону: одна колонка и настоящие вкладки.** На запущенном сайте стало видно, что дело не в красках, а в построении: узкая колонка с боковой панелью, обложка в 148 пикселей, потерянная рядом с пустым местом, четыре кнопки трёх разных весов и два заголовка друг под другом там, где у эталона вкладки. Теперь страница — одна колонка по центру шириной 1000 пикселей. Боковая панель не исчезла: справка о тайтле, команда проекта и виджеты площадки идут под оглавлением обычными разделами, поэтому ничего из настроенного владельцем не потерялось.

  Шапка получила обложку в 232 пикселя, название до 2.7rem, счётчики в одну строку с волосяными разделителями и собранные вместе кнопки нормального размера.

  Описание и оглавление стали вкладками — антиквой, с синей чертой под выбранной. Сделаны они двумя обычными панелями подряд: без JavaScript обе остаются на странице, поэтому список глав по-прежнему виден и поисковику, и читателю с выключенными скриптами. Ссылка на `#chapters` из шапки и из читалки открывает вкладку с главами вместо прокрутки к закрытой панели.

  Теги переехали из аннотации в собственный раздел, а атрибут `hidden` наконец сильнее правил темы с `display`.

## [0.7.0-beta] — 2026-08-27

### Removed

- **Bootstrap вынесен из темы.** Разметка использовала 62 класса из 2031, и 29 из них тема всё равно переопределяла своим CSS. За оставшиеся 33 класса и четыре поведения — модалка, панель сбоку, выпадающее меню, аккордеон — каждая страница платила 232 КБ стилей и 81 КБ скрипта, 30.8 и 23.7 КБ по проводу. Теперь это `assets/css/base.css` и `assets/js/ui.js`: те же имена классов и те же `data-bs-*`, поэтому ни один шаблон не правился и сторонняя разметка, написанная под фреймворк, продолжает работать. Вместе они весят 4.9 и 3.3 КБ в gzip против прежних 54.5 — минус два запроса на каждой странице и 53 КБ в пакете темы.

  Отдельная тонкость перехода: своё оформление тема писала не свойствами, а переменными фреймворка (`--bs-btn-bg`, `--bs-pagination-*`). Значения задавала она, а рисовал их Bootstrap — поэтому в `base.css` есть раздел, который эти переменные читает. Без него кнопки остались бы без фона и полей, а пагинация встала бы в столбик.

  Этим закрыт «framework bake-off» из планов: мерить пять кандидатов оказалось незачем, когда выяснилось, что действующий используется на 3.1%.

### Changed

- **Новый облик — библиотечная подача вместо приборной панели.** Белые полотна на холодном светлом фоне, антиква и в интерфейсе, и в тексте, одна синяя краска на все действия, оранжевая — только на оценку и ранг, волосяные разделители вместо карточек с тенями, кнопки-таблетками. `assets/css/look.css` переставляет токены темы, поэтому каталог, кабинет автора и панель управления меняют вид без единой строчки под каждый экран; отдельно правятся только те узлы, где построение другое: шапка тайтла теряет размытую картину ради ровной полосы, заголовки разделов становятся вкладками с чертой, а оглавление уходит в две колонки — пятьсот глав перестают быть километром прокрутки.

  Читалка сохраняет свои «бумаги»: как сайт, белая, сепия, ночь — это выбор читателя, и подменять его оформлением темы неправильно. Ограничена только мера строки, 40em, где глаз перестаёт терять начало следующей.

  Настройки Студии темы теперь подключаются последними в каскаде: свои цвета площадки новый облик не отнимает.
...
- **Скачивание книг закрывается ролью, а не только подпиской.** В панели управления, во вкладке «Настройки», появился список «Скачивание книг»: всем, любому вошедшему, по доступу PLUS, «PLUS или выбранные роли» и «только выбранным ролям», а под ним — галочки ролей площадки, включая те, что завёл плагин членства. Раньше выгрузка EPUB и FB2 была открыта каждому, кто нашёл ссылку: единственным ограничением был счётчик в десять файлов за десять минут. Кнопка и сам файл спрашивают одну и ту же функцию, поэтому прямая ссылка `?xin_export=epub` отвечает ровно тем же, что показывает страница, — кнопки нет, значит и файла нет. Закрытые главы, как и прежде, не попадают в книгу ни при каком режиме, а администратор скачивает всегда: иначе собственной настройкой можно было бы запереть самого себя. Плагину членства достаточно фильтра `xin_can_download`, тема при этом не правится. То же самое есть в кастомайзере — раздел «XI Novels: доступ к книгам».
...
- **Инструменты абзаца в читалке и озвучка голосами устройства.** Клик по абзацу главы открывает панель: закладка одной из четырёх красок, ссылка на конкретный абзац, цитата в обсуждение, предложенная правка с живым сравнением, чтение вслух с этого места. Голоса берутся из браузера — студия раскладывает их по языкам, отделяет установленные на устройстве от сетевых, даёт фильтр, скорость, тон, громкость и предпрослушивание; выбор запоминается. Из панели читалки можно прыгнуть к своей закладке. Спасибо HeavenlyCatCodes — вклад пришёл пул-реквестом.
- **Строки этого вклада переведены.** Они были написаны по-английски, а исходный язык темы — русский, поэтому на русском сайте панель и студия голосов остались бы английскими. Сорок три строки переведены на русский в исходниках и на английский с бразильским португальским в картах.
- **Три языка у обоих плагинов.** `xi-novel-import` и `xi-novel-manager` заведены в сборщик переводов: 119 и 92 строки, английский и бразильский португальский. Названия и описания плагинов в списке тоже меняются по языку WordPress — плагинам добавлен `Domain Path`.

### Changed

- **Отложенная глава в кабинете говорит, когда выйдет.** Раньше строка показывала «Черновик» и дату создания: по ней нельзя было понять ни того, что глава ждёт своего часа, ни когда он настанет. Теперь там значок «В очереди», дата выхода и сколько до неё осталось. Тема считает состояние сама и отдаёт его фильтром `xin_chapter_state`, поэтому штатные отложенные записи WordPress тоже подписаны правильно, а плагин очереди дописывает своё.
- **Расписание проекта переехало к автору.** Дни недели, время выхода и «из очереди выходят бесплатными» появились прямо в настройках проекта в кабинете, вместе со сводкой: сколько глав ждёт и когда выйдет ближайшая. Форма проекта получила пару хуков — `xin_novel_form_extra` и `xin_novel_form_save`.

### Experimental

- **Новая схема адресов глав — выключена по умолчанию.** Слаг главы собирался из одного названия, а названия повторяются: во втором проекте «Тишина» превращалась в `tishina-2`, в третьем — в `tishina-3`, и адрес переставал что-либо значить. Новая схема даёт `chapter-12-tishina` и селит главу внутри её проекта: `/novels/stantsiya-tishina/chapter-12-tishina/`. Уникальность считается внутри проекта, поэтому одинаковые названия в разных проектах остаются одинаковыми, а совпадение внутри одного получает номер. Старые адреса отвечают редиректом.

  Переезд меняет адрес каждой главы, поэтому включается вручную — иначе обычное обновление темы молча переписало бы все ссылки на площадке:

  ```php
  define( 'XIN_NESTED_CHAPTER_URLS', true );
  ```

  После включения — «Настройки → Постоянные ссылки → Сохранить», затем действие «Пересобрать адреса глав» в массовом управлении: оно переведёт уже существующие главы, а прежние адреса WordPress запомнит и продолжит редиректить.

## [0.3.4-beta] — 2026-08-22

### Fixed

- **Строки рейтинга обрезались на телефоне.** Заголовок в строке выводится в одну строку с многоточием, а элемент грида не сжимается уже своего минимального содержимого — поэтому строка вырастала до 604px при экране 360px, и колонка со счётом просто уезжала под обрез. Теперь сжиматься разрешено всей цепочке, и строка укладывается в экран на 360, 768 и 1280.
- **Ссылка «Рейтинг» вела на старый каталог, пока страница уже существовала.** Тема сбрасывает кэш один раз при смене версии, на `init` с приоритетом 30, а страницы заводятся раньше, на приоритете 20. Если файлы темы приезжают на сервер не одним куском, сброс успевает израсходоваться до того, как страница появилась: свежие страницы показывали новую ссылку, а главная навсегда оставалась в кэше со старой. Сброс кэша вынесен из проверки версии и вызывается ещё и тогда, когда страница действительно создана.

## [0.3.3-beta] — 2026-08-22

### Added

- **Рейтинг стал отдельной страницей, а не каталогом с другой сортировкой.** Шаблон *Рейтинг* и страница `/ranking/`, которая заводится сама. Три доски — по оценке, по просмотрам и по числу глав, — три окна времени и фильтр по жанру; первая тройка стоит на подиуме, остальные идут строками с местом, автором и полосой относительно лидера. Оценка считается взвешенно: пока у тайтла меньше пяти голосов, его средняя подтягивается к средней по площадке, поэтому одна пятёрка не обгоняет четыре сотни честных оценок. Доски считаются раз в десять минут и пересобираются, когда кто-то поставил оценку или сохранил тайтл или главу. Все ссылки «Рейтинг» — в шапке, на главной и в блоке разделов — ведут на неё, а на сайте, где страницы ещё нет, откатываются на прежний отсортированный каталог.
- **Массовое управление тайтлами — отдельный плагин.** `xi-novel-manager`, экран *Новеллы → Массовое управление*. Поиск по названию и фильтры — владелец, жанр, статус тайтла, состояние публикации, наличие обложки, 18+ — складываются в обычную ссылку, которую можно сохранить или передать. Отмеченное галочками (диапазон выделяется с Shift) или, если найденного больше страницы, всё найденное сразу уходит в одно из действий: публикация и черновики, статус тайтла, 18+, жанры и метки — добавить, убрать или заменить списком через запятую, — владелец и команда перевода, одна обложка на пачку, ранний доступ PLUS на все главы тайтла, выгрузка в CSV и корзина с безвозвратным удалением. Удаление требует права `delete_others_posts`, остальное — `edit_others_posts`, и каждый идентификатор ещё раз проверяется через `current_user_can( 'edit_post' )`, так что подменённая форма чужой тайтл не тронет. Удаление тайтла уносит с собой его главы — они отдельные записи и иначе остались бы висеть без входа.

### Fixed

- **Фильтр каталога уводил на главную.** Форма фильтра отправляется методом GET, а такая форма заменяет строку запроса своего адреса собственными полями. На сайте с «простыми» ссылками, где каталог живёт по адресу `/?post_type=novel`, выбор сортировки терял `post_type` — и вместо отфильтрованного каталога открывалась главная. Заодно терялся язык. Теперь адрес формы разбирается и его аргументы возвращаются скрытыми полями; та же конструкция была в поиске по пользователям в панели управления и исправлена вместе с ней.
- **Надписи на обложках было не прочесть.** Метки *Завершён* и *18+* лежат поверх произвольной картинки, а фон у них был полупрозрачной заливкой в 12%: цвет обложки просвечивал, и текст сливался с ней. Над обложкой метки стали непрозрачными и несут свой фон — контраст 5,6:1 у *18+* и 12,7:1 у статуса вместо почти неразличимого.
- **Пустая половина в блоке «Топ-авторы».** Два блока стояли в сетке из двух равных половин, растягивающей каждый по высоте самого длинного: короткий список авторов оставлял рядом с новостями сначала высокий пустой прямоугольник, а после первой правки — половину экрана пустоты. Теперь авторы занимают колонку по своему размеру, лента новостей забирает остальное, и каждый блок держит свою высоту. Карточки авторов внутри тоже перестали делиться ровно на три и раскладываются по ширине.
- **Наведение выглядело как выбранный раздел.** Пункты меню в шапке красили одинаковой серой таблеткой и наведение, и текущий раздел. Наведение осталось нейтральным, текущий раздел стал акцентным — и встроенное меню теперь само определяет, где вы находитесь: без назначенного меню WordPress не помечал ничего, и подсвеченным выглядел только тот пункт, на котором стояла мышь.

### Changed

- **Списки фильтров рисует тема, а не операционная система.** Сортировка и статус в каталоге были обычными `select`, а их список открывает система — серое окно поверх страницы, мимо оформления сайта. Сам `select` остался в форме и по-прежнему работает со отключёнными скриптами; поверх него появился список в оформлении темы, с клавиатурой (стрелки, Home, End, Esc) и отметкой выбранного. Он тоже удерживается в пределах экрана.
- Ссылки «Все» в заголовках блоков получили область нажатия и подсветку, а не были просто мелким серым текстом.
