...
Версии спутников: импорт глав 1.2.0, управление тайтлами 1.1.1, студия темы 0.4.0.
...
- **Раздел комиксов включается в настройках темы.** «Внешний вид → Настроить → XIN-Com: разделы». По умолчанию выключен: раздел добавляет сайту адреса `/comics/` и пункт в шапке, а площадке, издающей только текст, всё это не нужно. Выключение ничего не удаляет и не возвращает комиксы в каталог новелл — фильтр разделов работает независимо от модуля.
- **Выбор сервера картинок в читалке комикса.** Если хранилище тормозит или недоступно из чьей-то сети, читатель переключается на зеркало сам, не дожидаясь, пока это заметят на стороне сайта. Тема объявляет фильтр источников и работает без плагина; зеркала настраиваются в плагине импорта, по одному на строку. Зеркало предлагается только тем главам, что выгружены целиком: дыра в середине главы хуже, чем отсутствие выбора.
- **Раздел комиксов рядом с разделом новелл.** В шапке появился переключатель, у каждого раздела своя главная: новеллы остаются на `/`, комиксы живут на `/comics/`, каталог — на `/comics/catalog/`, тайтл и глава — на `/comics/<тайтл>/` и `/comics/<тайтл>/<глава>/`. Формат тайтла — мета `_xin_format`, а не второй тип записи: каталог, рейтинги, библиотека, избранное, оценки, роли и студия ключуются на `novel`, и второй тип означал бы вторую копию каждой из этих вещей. Плата за такое решение — риск протечки, поэтому фильтр ровно один, в `pre_get_posts`, и умолчание в нём текстовое: тайтл без меты считается новеллой, и раздел новелл остаётся тем же, чем был.
- **Читалка комикса.** Лентой вниз для вебтуна или постранично, у манги справа налево; настраиваются ширина кадра и зазор, прогресс считается по номеру кадра, позиция и настройки переживают перезаход. Текстовую читалку, словарь и озвучку страница комикса не подключает: они считают по тексту, которого здесь нет.
- **Страницы комиксов можно выгружать в S3-совместимое хранилище.** Настройки на экране «Инструменты → Импорт глав»: адрес, бакет, регион, доступ, папка, домен CDN, проверка связи и догрузка того, что уже лежит на диске. Запрос подписывается по AWS Signature V4 без SDK. Выгружаются только страницы комиксов; локальная копия удаляется по явной просьбе и только после успешной выгрузки. Ключ и секрет читаются из констант `XNI_S3_KEY` и `XNI_S3_SECRET`, если они заданы в `wp-config.php` — так доступа не остаётся в базе.

### Changed

- **Витрина раздела комиксов и страница тайтла собраны заново.** На витрине — афиша во весь экран вместо плитки равнозначных обложек, неделя выходов и лента, сгруппированная по тайтлам, а не по главам: иначе тайтл, выложивший вечером три главы, занимает собой всю ленту. День недели берётся из дат последних глав, а не из отдельного поля — заполняемое руками поле расходилось бы с выходами молча. Страница тайтла получила первый экран с одной кнопкой и вкладки: описание, главы, похожее; список глав, сортировка, поиск и сворачивание описания работают на тех же механизмах, что и страница новеллы.
- **Тема переименована из XI Novels в XIN-Com.** Сменилось всё, что видно снаружи: имя темы, каталог `themes/xi-novels` → `themes/xin-com`, text domain `xi-novels` → `xin-com` и дескрипторы ассетов вроде `xi-novels-skin`. Префиксы функций `xin_` и ключи меты `_xin_` остались как были — они и так читаются как XIN, а их правка потребовала бы миграции базы на живых сайтах. Обновлённые сайты после обновления увидят тему как новую и потеряют настройки кастамайзера: опцию `theme_mods_xi-novels` нужно скопировать в `theme_mods_xin-com`, а дочерним темам — поменять `Template:` на `xin-com`. Записи этого журнала за прошлые релизы оставлены под старым именем: они выходили именно так.
